...
# 把df写进txt文件中，以unicode编码，不要使用csv格式，否则会出现乱码
df.to_csv('C:\\Users\\A\\Desktop\\二手房\\四个小区_20230826_161919.txt', sep='\t', encoding='utf-8')
...
# 5. 对所有同一个年份同一个显示小区名的数据，计算交易周期
grouped_result = grouped.groupby(['成交年份', '显示小区名'])['交易周期'].median().unstack()

# 6. 横坐标是年份，纵坐标是交易周期，图例是显示小区名，请用折线图进行可视化
