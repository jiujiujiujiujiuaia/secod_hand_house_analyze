...
# 4. 计算每一个年份和户型结构的挂牌价格和成交价格差值的平均值
data['挂牌价格和成交价格差值'] = data['挂牌价格'] - data['成交价格']

# 对下面计算中位数
# grouped_data = data.groupby(['year', '显示户型结构'])['挂牌价格和成交价格差值'].median().reset_index()
grouped_data = data.groupby(['year', '显示户型结构'])['挂牌价格和成交价格差值'].mean().reset_index()

# 5. & 6. 使用折线图进行可视化
plt.figure(figsize=(12, 8))
sns.lineplot(data=grouped_data, x='year', y='挂牌价格和成交价格差值', hue='显示户型结构', palette='tab10')
plt.title("年度挂牌价格和成交价格差值与户型结构关系")
plt.xlabel("年份")
