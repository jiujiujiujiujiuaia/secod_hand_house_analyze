...
# 4. 计算每一个年份和楼层类型的交易周期平均值
grouped_data = data.groupby(['year', '显示楼层类型'])['交易周期'].mean().reset_index()

# 5. & 6. 使用折线图进行可视化
plt.figure(figsize=(12, 8))
sns.lineplot(data=grouped_data, x='year', y='交易周期', hue='显示楼层类型', palette='tab10')
plt.title("年度交易周期与楼层类型关系")
plt.xlabel("年份")
plt.ylabel("交易周期")
